...
public class SdlService extends Service {
	...
	private static int pressCountGood = 0;
	private static int pressCountBad = 0;
	...
	//private static final int TCP_PORT = 10893;
	private static final String DEV_MACHINE_IP_ADDRESS = "m.sdl.tools";
	private static final int TCP_PORT = 17744;
	//private static final String DEV_MACHINE_IP_ADDRESS = "10.0.0.1";
	// variable to create and call functions of the SyncProxy
	private SdlManager sdlManager = null;

	private GPSData gpsSaveBuff = new GPSData();
	private Double  accSaveBuff = null;
	private Double  sppedSaveBuff = null;
				...
				public void onStart() {
					...
					sdlManager.addOnRPCNotificationListener(FunctionID.ON_VEHICLE_DATA, new OnRPCNotificationListener() {
						...
						public void onNotified(RPCNotification notification) {
							...
							Double speedMeter = onVehicleDataNotification.getSpeed();
							Double accPedal = onVehicleDataNotification.getAccPedalPosition();
							...
							//スピード表示
							if((speedMeter != null) || (accPedal != null)){
								checkSlamBrake(speedMeter,accPedal);
							}
							...
							if(onVehicleDataNotification.getGps() != null){
								setGpsInfo(onVehicleDataNotification.getGps());
							}
							// アクセルペダルの踏量
	...
	private void setGpsInfo(GPSData rawData){
		gpsSaveBuff = rawData;
	}

	public String getGpsLong(){
		return gpsSaveBuff.getLongitudeDegrees().toString();
	}

	public String getGpsLati() {
		return  gpsSaveBuff.getLatitudeDegrees().toString();
	}

		// 急ブレーキ判定くん
	private void checkSlamBrake(Double speed, Double accPedal) {
		sppedSaveBuff = speed;
	}
	...
	private void setDisplayGraphicWithTextButtons(){
		...
		setDisplayLayoutRequest.setDisplayLayout(PredefinedLayout.GRAPHIC_WITH_TILES.toString());
				...
				if(((SetDisplayLayoutResponse) response).getSuccess()){
					...
					softButtonObject2.setOnEventListener(new SoftButtonObject.OnEventListener() {
						...
						public void onPress(SoftButtonObject softButtonObject, OnButtonPress onButtonPress) {
							...
							send.sendEvaluation(EvaluationType.Good);
							onClickGoodButton();
	...
	private void onClickGoodButton(){
		IDataSender send = new DataSender();
		send.sendEvaluationsWithGps(EvaluationType.Good,getGpsLong(),getGpsLati());
	}

	private void onClickBadButton(){
		IDataSender send = new DataSender();
		send.sendEvaluationsWithGps(EvaluationType.Bad,getGpsLong(),getGpsLati());
	}
